...
module ApplicationHelper
  def add_stylesheet(
    filename = '', opts = { media: 'all', 'data-turbolinks-track': 'reload' }
  )
    if (
         Rails.application.assets ||
           ::Sprockets::Railtie.build_environment(Rails.application)
       )
         .find_asset(filename + '.css')
      stylesheet_link_tag(filename, opts)
    end
  end

  def add_javascript(
    filename = '', opts = { 'data-turbolinks-track': 'reload' }
  )
    if (
         Rails.application.assets ||
           ::Sprockets::Railtie.build_environment(Rails.application)
       )
         .find_asset(filename + '.js')
  ...
  end # rubocop:todo Naming/PredicateName
end
